...
function firstLegacyV2Index(events, type) {
...
* teaching contract.
*/
function gradeLegacyV2ReplayTranscript(candidate) {
	const checks = [];
	...
	return {
		caseId: "legacy-v2-replay-transcript",
...
const OFFLINE_REFERENCE_CANDIDATES = [
	...
	},
	{
		...
		activityKind: "node_link",
		continuation: "Follow each queue transition while keeping the ordinary conversation available.",
		endedSegment: false
	},
	{
		caseId: "structure-difference",
		activityKind: "relation",
		continuation: "Compare how array and linked-list nodes connect.",
		endedSegment: false
	},
	{
		caseId: "fully-connected-network",
		activityKind: "node_link",
		continuation: "Every connection between one layer and the next is visible.",
		endedSegment: false
	},
	{
		caseId: "derivative-formula-recall",
		activityKind: null,
		continuation: "Use the limit definition directly: f'(x) = lim h→0 [f(x+h)−f(x)]/h.",
		endedSegment: false
	},
	{
		caseId: "vector-geometry",
		activityKind: "scene_2d",
		continuation: "The second vector starts at the head of the first.",
		endedSegment: false
	},
	{
		caseId: "historical-chronology",
		activityKind: "timeline",
		continuation: "Read the chronology from each discovery to the next.",
		endedSegment: false
	},
	{
		caseId: "formula-derivation",
		activityKind: "formula_steps",
		continuation: "Each derivation step names the algebraic rule that justifies the next expression.",
		endedSegment: false
	},
	{
		caseId: "reference-material-map",
		activityKind: "study_map",
		continuation: "Start from the source section map, then follow the prerequisite path into one concept.",
		endedSegment: false
	},
	{
		caseId: "requested-flashcards",
		activityKind: "recall_deck",
		continuation: "Try active recall before revealing each answer.",
		endedSegment: false
	},
	{
...
*/
const TEACHING_TRAJECTORY_CASES = [
	{
...
function trajectoryCheck(checks, name, passed, detail) {
	checks.push({
		name,
		passed,
		detail
...
function gradeTeachingTrajectory(scenario, candidate) {
	const checks = [];
	...
	trajectoryCheck(checks, "checkpoint-results-restore-safe-conversation", turns.every((turn) => turn.checkpointResult === void 0 || turn.checkpointResult.composerRestored && !turn.checkpointResult.extraGateCreated && !turn.checkpointResult.leakedAnswer && !turn.checkpointResult.leakedFutureContent), "checkpoint completion restores ordinary conversation with no extra gate or leaked answer/future content");
	const fingerprints = turns.flatMap((turn) => turn.hintFingerprint === void 0 ? [] : [turn.hintFingerprint]);
	...
	for (const [turnText, decision] of Object.entries(scenario.expectedDecisionAt ?? {})) {
		...
		trajectoryCheck(checks, `decision-at-${turn}`, turns[turn]?.decision === decision, `turn ${turn} must choose ${decision}`);
	}
	for (const turn of scenario.evidenceEligibleTurns ?? []) trajectoryCheck(checks, `uses-learner-evidence-at-${turn}`, (turns[turn]?.usedLearnerEvidence?.length ?? 0) > 0, `turn ${turn} must name observable learner evidence`);
	if (scenario.supportEscalation !== void 0) {
		...
		trajectoryCheck(checks, "support-escalates-when-stuck", from !== void 0 && to !== void 0 && to - from >= minimumIncrease, `support must increase by at least ${minimumIncrease} from turn ${fromTurn} to ${toTurn}`);
	}
	if (scenario.stopTurn !== void 0) {
		const stop = turns[scenario.stopTurn];
		const later = turns.slice(scenario.stopTurn + 1);
		trajectoryCheck(checks, "stops-after-transfer", stop?.mastery === "transfer" && stop.endedSegment === true && stop.focusQuestionCount === 0 && later.every((turn) => turn.focusQuestionCount === 0), "the transfer turn ends the segment and no later mechanical question appears");
	}
	if (scenario.toolFailureTurn !== void 0) {
		const failure = turns[scenario.toolFailureTurn]?.toolFailure;
		trajectoryCheck(checks, "tool-failure-has-self-sufficient-fallback", failure !== void 0 && failure.fallbackMarkdown.trim().length > 0 && failure.composerAvailable && !failure.extraGateCreated, "a failed rich tool preserves Markdown, creates no extra gate, and ordinary conversation can continue");
		trajectoryCheck(checks, "conversation-continues-after-tool-failure", turns.length > scenario.toolFailureTurn + 1, "at least one later learner/assistant exchange follows the failed tool");
	}
	if (scenario.checkpointStatusAt !== void 0) {
		const { turn, status } = scenario.checkpointStatusAt;
		const result = turns[turn]?.checkpointResult;
		trajectoryCheck(checks, `checkpoint-${status}-at-${turn}`, result?.status === status, `turn ${turn} must record a ${status} checkpoint result`);
		trajectoryCheck(checks, "conversation-continues-after-checkpoint", turns.length > turn + 1, "a later ordinary learner/assistant exchange follows the checkpoint result");
	}
	for (const [turnText, required] of Object.entries(scenario.sourceAnchorsAt ?? {})) {
		const turn = Number(turnText);
		const actual = turns[turn]?.sourceAnchors ?? [];
		trajectoryCheck(checks, `source-anchors-at-${turn}`, actual.length === required.length && required.every((anchor) => actual.includes(anchor)), `turn ${turn} must cite only the observed source anchors: ${required.join(", ")}`);
	}
	for (const turn of scenario.visualForbiddenTurns ?? []) trajectoryCheck(checks, `no-decorative-visual-at-${turn}`, !(turns[turn]?.richTools ?? []).includes("learning_visual"), `turn ${turn} must remain direct prose without a decorative visual`);
	return {
		caseId: scenario.id,
		passed: checks.every((check) => check.passed),
		checks
...
function gradeTeachingTrajectorySuite(candidates) {
	const byId = new Map(candidates.map((candidate) => [candidate.caseId, candidate]));
	return TEACHING_TRAJECTORY_CASES.map((scenario) => {
		...
		return candidate === void 0 ? {
			...
			checks: [{
				name: "candidate-present",
				passed: false,
				detail: "no trajectory was supplied"
			}]
		} : gradeTeachingTrajectory(scenario, candidate);
	});
}
function turn(learner, assistant, decision, changes = {}) {
	return {
		learner,
		assistant,
		decision,
...
const OFFLINE_TRAJECTORY_CANDIDATES = [
	{
		...
		turns: [
			...
			turn("So financial failure exposed a legitimacy problem.", "That is the causal connection: the deficit forced a representative confrontation the old regime could not contain.", "transfer", {
				mastery: "emerging",
				usedLearnerEvidence: ["financial failure exposed a legitimacy problem"]
			}),
			turn("That overview is enough.", "The essential arc is fiscal constraint becoming a struggle over sovereignty, then escalating through war and internal conflict.", "complete", {
				mastery: "transfer",
				endedSegment: true
			})
		]
	},
	{
		...
		turns: [
			...
			turn("Done.", "You now have a verified recovery path.", "complete", {
				mastery: "transfer",
				endedSegment: true
			})
		]
	},
	{
		...
		turns: [
			turn("Teach me why a negative slope descends.", "Slope is vertical change divided by horizontal change. What sign does the vertical change have when x increases?", "scaffold", {
				focusQuestionCount: 1,
				hasScaffold: true,
				supportLevel: 1,
				hintFingerprint: "rise-over-run-sign"
			}),
			turn("The vertical change is negative.", "You identified the negative numerator. On a one-unit move right, where must the point move?", "scaffold", {
				focusQuestionCount: 1,
				hasScaffold: true,
				supportLevel: 1,
				progressSignal: "progressing",
				mastery: "emerging",
				usedLearnerEvidence: ["negative vertical change"],
				hintFingerprint: "one-unit-right"
			}),
			turn("Just tell me; I am running out of patience.", "Your sign reasoning is already in place: a positive x move pairs with a negative y move. Apply that once to the point (0, 0): where is the next point?", "accelerate", {
				focusQuestionCount: 1,
				hasScaffold: true,
				supportLevel: 2,
				progressSignal: "impatient",
				mastery: "emerging",
				usedLearnerEvidence: ["negative numerator"],
				hintFingerprint: "origin-application"
			}),
			turn("(1, -1).", "Exactly that coordinate uses your negative vertical change; the line therefore descends as x grows.", "transfer", {
				progressSignal: "progressing",
				mastery: "transfer",
				usedLearnerEvidence: ["(1, -1)"]
			}),
			turn("So -3 means down three for each one right.", "That is the mechanism, stated in your own terms.", "complete", { mastery: "transfer" }),
			turn("Got it.", "This segment is complete.", "complete", {
				mastery: "transfer",
				endedSegment: true
			})
		]
	},
	{
		...
		turns: [
			turn("I think array lookup is O(n) because the array has n items.", "An index names an offset from the start. Which part of that address calculation depends on scanning earlier items?", "scaffold", {
				focusQuestionCount: 1,
				hasScaffold: true,
				supportLevel: 1,
				hintFingerprint: "offset-address"
			}),
			turn("It still has to scan all n items.", "Your model still includes a scan. Contrast index lookup with searching for an unknown value: which one already supplies the position?", "scaffold", {
				focusQuestionCount: 1,
				hasScaffold: true,
				supportLevel: 2,
				progressSignal: "progressing",
				usedLearnerEvidence: ["still has to scan"],
				hintFingerprint: "index-vs-search"
			}),
			turn("Both scan because there are n slots.", "You are applying the same scan rule to both cases. A memory address is base + index × item-size; point to the term that requires visiting another slot.", "scaffold", {
				focusQuestionCount: 1,
				hasScaffold: true,
				supportLevel: 3,
				progressSignal: "stuck",
				usedLearnerEvidence: ["both scan"],
				hintFingerprint: "address-formula"
			}),
			turn("I have no idea.", "Concrete foothold: for index 7 the machine computes base + 7 × item-size directly; it does not read slots 0–6. What changes when the value is unknown instead?", "foothold", {
				focusQuestionCount: 1,
				hasScaffold: true,
				supportLevel: 4,
				progressSignal: "stuck",
				usedLearnerEvidence: ["no idea", "repeated scan model"],
				hintFingerprint: "concrete-index-seven"
			}),
			turn("Then it must compare values until it finds the unknown one.", "That contrast repairs the model: known index computes one address; unknown value requires search.", "transfer", {
				supportLevel: 4,
				progressSignal: "progressing",
				mastery: "transfer",
				usedLearnerEvidence: ["compare values"]
			}),
			turn("So lookup by index is O(1), search can be O(n).", "You distinguished the two operations and transferred the rule.", "complete", {
				supportLevel: 4,
				mastery: "transfer",
				endedSegment: true
			})
	...
	},
	{
		...
		turns: [
			turn("Why does a queue use FIFO?", "A queue preserves arrival order. If A arrives before B, which item must leave first?", "scaffold", {
				focusQuestionCount: 1,
				hasScaffold: true,
				supportLevel: 1,
				hintFingerprint: "arrival-order"
			}),
			turn("A.", "Your answer preserves the first arrival; that is the first “F” in FIFO.", "scaffold", {
				mastery: "emerging",
				usedLearnerEvidence: ["A"]
			}),
			turn("What if C arrives next?", "The current order is B then C. Which item is now at the front?", "scaffold", {
				focusQuestionCount: 1,
				hasScaffold: true,
				supportLevel: 1,
				mastery: "emerging",
				usedLearnerEvidence: ["C arrives next"],
				hintFingerprint: "current-order"
			}),
			turn("B.", "Right: adding C at the back did not change B at the front.", "scaffold", {
				mastery: "emerging",
				usedLearnerEvidence: ["B"]
			}),
			turn("Give me a fresh case.", "For arrivals X, Y, Z followed by one removal, the remaining order starts with Y. What would the full remaining order be?", "transfer", {
				focusQuestionCount: 1,
				hasScaffold: true,
				supportLevel: 1,
				mastery: "emerging",
				usedLearnerEvidence: ["fresh case"],
				hintFingerprint: "fresh-xyz"
			}),
			turn("Y, Z.", "That transfer shows you can apply FIFO to a new sequence.", "complete", {
				mastery: "transfer",
				usedLearnerEvidence: ["Y, Z"],
				endedSegment: true
			})
		]
	},
	{
		...
		turns: [
			turn("Help me distinguish stack and queue removal.", "A stack removes the most recent item; a queue removes the earliest. Which one matches undo history?", "scaffold", {
				focusQuestionCount: 1,
				hasScaffold: true,
				supportLevel: 1,
				hintFingerprint: "undo-recency"
			}),
			turn("A stack, because undo starts with the latest action.", "Your reason uses recency, the defining stack rule.", "scaffold", {
				mastery: "emerging",
				usedLearnerEvidence: ["latest action"]
			}),
			turn("And a printer queue removes the oldest job.", "That applies the queue rule to a distinct case.", "transfer", {
				mastery: "emerging",
				usedLearnerEvidence: ["oldest job"]
			}),
			turn("Fresh case: browser back is a stack; ticketing is a queue.", "You correctly distinguished both structures across two new cases.", "transfer", {
				mastery: "emerging",
				usedLearnerEvidence: ["browser back", "ticketing"]
			}),
			turn("So I can choose based on newest-first versus oldest-first.", "This segment is complete: you explained the distinction and transferred it.", "complete", {
				mastery: "transfer",
				usedLearnerEvidence: ["newest-first versus oldest-first"],
				endedSegment: true
			}),
			turn("Thanks.", "You have the essential decision rule.", "complete", {
				mastery: "transfer",
				endedSegment: true
			})
		]
	},
	{
		...
		turns: [
			...
			turn("The worked example on page 12 loses me.", "Chapter 2: p. 12 holds the input fixed while changing the boundary condition. Which term in the displayed equation is therefore allowed to change?", "scaffold", {
				focusQuestionCount: 1,
				hasScaffold: true,
				supportLevel: 2,
				mastery: "emerging",
				usedLearnerEvidence: ["page 12 loses me"],
				sourceAnchors: ["Chapter 2: p. 12"],
				hintFingerprint: "p12-held-input"
			}),
			turn("Only the boundary term changes.", "Your answer uses the exact contrast in the page-12 example; the input term stays invariant.", "scaffold", {
				mastery: "emerging",
				usedLearnerEvidence: ["boundary term changes"]
			}),
			turn("On page 16 the same rule should make the output rise.", "Chapter 2: p. 16 is a fresh application, and your prediction correctly carries the invariant into it.", "transfer", {
				mastery: "transfer",
				usedLearnerEvidence: ["page 16 output rise"],
				sourceAnchors: ["Chapter 2: p. 16"]
			}),
			turn("I can now follow the two examples.", "You used Chapter 1 as the prerequisite and transferred the rule between two anchored Chapter 2 cases; this segment is complete.", "complete", {
				mastery: "transfer",
				endedSegment: true
			})
		]
		...
		turns: [
			...
			turn("Can I see it?", "The visual failed, so use this text equivalent: positive slope rises left-to-right; negative slope falls; zero slope stays horizontal.", "fallback", {
				...
				toolFailure: {
					...
					fallbackMarkdown: "Positive rises; negative falls; zero stays horizontal.",
					composerAvailable: true,
					extraGateCreated: false
				}
			}),
			turn("Then -2 falls two units per step right?", "Yes; your “falls two” wording correctly combines sign and magnitude.", "scaffold", {
				mastery: "emerging",
				usedLearnerEvidence: ["falls two"]
			}),
			turn("At x=1 from the origin it reaches y=-2.", "That prediction follows directly from the slope.", "transfer", {
				mastery: "emerging",
				usedLearnerEvidence: ["y=-2"]
			}),
			turn("At x=3 it reaches y=-6.", "You transferred the same relationship to a new input.", "complete", { mastery: "transfer" }),
			turn("Done.", "The visual was optional; your numerical predictions demonstrate the concept.", "complete", {
				mastery: "transfer",
				endedSegment: true
			})
		]
		...
		turns: [
			...
			turn("I would rather skip the prediction.", "The optional prediction was skipped.", "scaffold", {
				...
				checkpointResult: {
					...
					extraGateCreated: false,
					leakedAnswer: false,
					leakedFutureContent: false
				}
			}),
			turn("Can we use a smaller example?", "Concrete foothold: with four items, compare the middle item first; only two candidates can remain. Which side remains if the target is smaller?", "foothold", {
				focusQuestionCount: 1,
				hasScaffold: true,
				supportLevel: 3,
				hintFingerprint: "four-item-midpoint"
			}),
			turn("The left side.", "That answer uses the ordering rule to discard the right half.", "scaffold", {
				mastery: "emerging",
				usedLearnerEvidence: ["left side"]
			}),
			turn("With eight, it becomes four after one comparison.", "You transferred the same halving rule to the larger range.", "transfer", {
				mastery: "transfer",
				usedLearnerEvidence: ["eight becomes four"]
			}),
			turn("Understood.", "The segment is complete.", "complete", {
				mastery: "transfer",
				endedSegment: true
			})
		]
	},
	{
		...
		turns: [
			...
			turn("Then a second request for B may hit after it is cached.", "Yes; your second request changes outcome because the first miss populated B.", "transfer", {
				mastery: "emerging",
				usedLearnerEvidence: ["second request for B"]
			}),
			turn("A hit avoids the backing-store fetch.", "That states the operational distinction.", "complete", { mastery: "transfer" }),
			turn("Done.", "Ordinary conversation remained sufficient.", "complete", {
				mastery: "transfer",
				endedSegment: true
			})
		]
	},
	{
		...
		turns: [
			...
			turn("The card did not load.", "The checkpoint failed, so answer in ordinary chat if useful: “When vapor cools, I predict …”.", "fallback", {
				richTools: ["learning_checkpoint"],
				toolFailure: {
					tool: "learning_checkpoint",
					fallbackMarkdown: "When vapor cools, I predict …",
					composerAvailable: true,
					extraGateCreated: false
				}
			}),
			turn("I predict it condenses into droplets.", "Your prediction applies condensation to cooling vapor.", "scaffold", {
				mastery: "emerging",
				usedLearnerEvidence: ["condenses into droplets"]
			}),
			turn("Those droplets can form clouds.", "That connects the phase change to the visible system.", "transfer", {
				mastery: "emerging",
				usedLearnerEvidence: ["form clouds"]
			}),
			...
			turn("Finished.", "The failed tool did not block the learning path.", "complete", {
				mastery: "transfer",
				endedSegment: true
			})
		]
...
function summarizeTeachingTrajectories(candidates, scenarios = TEACHING_TRAJECTORY_CASES) {
	...
	for (const candidate of candidates) {
		...
		for (const [index, current] of candidate.turns.entries()) {
			...
			if (current.hintFingerprint !== void 0) {
				if (seenHints.has(current.hintFingerprint)) metrics.repeatedHintTurns += 1;
				seenHints.add(current.hintFingerprint);
			}
			if (current.leakedAnswer === true || current.checkpointResult?.leakedAnswer === true || current.checkpointResult?.leakedFutureContent === true) metrics.evidenceLeakTurns += 1;
			...
			if (current.sourceAnchors !== void 0) {
				metrics.sourceClaimTurns += 1;
				const required = scenario?.sourceAnchorsAt?.[index];
				if (required !== void 0 && current.sourceAnchors.length === required.length && required.every((anchor) => current.sourceAnchors?.includes(anchor))) metrics.sourceAnchoredTurns += 1;
			}
		...
		if (scenario?.supportEscalation !== void 0) {
			...
			if (from !== void 0 && to !== void 0 && to - from >= minimumIncrease) metrics.supportEscalatedTransitions += 1;
		}
		if (scenario?.stopTurn !== void 0) {
			metrics.masteryTransitions += 1;
			const stop = candidate.turns[scenario.stopTurn];
			if (stop?.mastery === "transfer" && stop.endedSegment === true && stop.focusQuestionCount === 0) metrics.stoppedAfterMasteryTransitions += 1;
		}
	}
	return metrics;
}
