...
function ParameterVisual({ activity }) {
    if (activity.visual?.kind !== 'parameter')
        return null;
    return (_jsxs("section", { className: css.roundParameter, children: [_jsx("div", { className: css.roundParameterValues, children: activity.visual.parameters.map(parameter => (_jsxs("span", { children: [_jsx("strong", { children: parameter.label }), " ", parameter.initial] }, parameter.id))) }), _jsx("div", { className: css.roundCurveList, children: activity.visual.curves.map(curve => _jsx("span", { children: curve.label }, curve.id)) })] }));
}
function StructureVisual({ activity }) {
    if (activity.visual?.kind !== 'structure')
        return null;
    return (_jsx("section", { className: css.roundStructure, children: [activity.visual.left, activity.visual.right].map(side => (_jsxs("div", { children: [_jsx("h3", { children: side.title }), _jsx("ul", { children: side.items.map(item => _jsx("li", { children: item.label }, item.id)) })] }, side.title))) }));
}
function CurrentVisual({ activity, final }) {
    ...
    if (activity.visual.kind === 'parameter')
        return _jsx(ParameterVisual, { activity: activity });
