import { TRANSPORT_PROTOCOL_V2, TRANSPORT_PROTOCOL, parseLearningActivity, parseLearningActivityV2, } from "./protocol.js";
...
export function learningWaitQuestionId(waitId) {
    if (!/^[A-Za-z0-9_-]{1,128}$/.test(waitId))
        throw new Error('waitId must be a URL-safe opaque token');
    return `${WAIT_QUESTION_ID_PREFIX}${waitId}`;
}
export function decodeLearningWaitQuestionId(value) {
    if (typeof value !== 'string' || !value.startsWith(WAIT_QUESTION_ID_PREFIX))
        return undefined;
 ...
 */
export function encodeLearningWaitDetail(input) {
    const envelope = { transport: TRANSPORT_PROTOCOL_V2, ...input };
    ...
    if (activity.phase === 'question' && activity.lessonToken !== undefined
        && activity.lessonToken !== envelope.lessonToken)
        throw new Error('wait projection token mismatch');
    return `${WAIT_MARKER_PREFIX}${encodeBase64Url(JSON.stringify({ ...envelope, activity }))}${MARKER_SUFFIX}\n${activity.fallbackMarkdown}`;
}
export function decodeLearningWaitDetail(detail) {
    if (typeof detail !== 'string' || !detail.startsWith(WAIT_MARKER_PREFIX))
        return undefined;
    ...
    try {
        ...
        if (parsed.transport !== TRANSPORT_PROTOCOL_V2
            ...
            || (parsed.phase !== 'question' && parsed.phase !== 'reveal'))
            return undefined;
        const activity = parseLearningActivityV2(parsed.activity);
        if (activity.phase !== parsed.phase || activity.seq !== parsed.seq)
            return undefined;
        if (activity.phase === 'reveal'
            && (activity.lessonToken !== parsed.lessonToken || activity.roundToken !== parsed.roundToken))
            return undefined;
        if (activity.phase === 'question' && activity.lessonToken !== undefined
            && activity.lessonToken !== parsed.lessonToken)
            return undefined;
        return {
